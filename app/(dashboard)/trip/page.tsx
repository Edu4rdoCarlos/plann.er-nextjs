...
export default function TripPage() {
  ...
  const getTripStatus = (startDate: string | Date, endDate: string | Date) => {
    ...
    if (now < start) {
      return { status: "upcoming", label: "Próxima", color: "bg-blue-500" };
    } else if (now > end) {
      return { status: "completed", label: "Concluída", color: "bg-gray-500" };
    } else {
      return { status: "ongoing", label: "Em andamento", color: "text-lime-300" };
    }
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-zinc-400 text-lg">
            {trips?.length ? `${trips.length} viagem${trips.length > 1 ? 'ns' : ''} planejada${trips.length > 1 ? 's' : ''}` : 'Nenhuma viagem ainda'}
          </p>
        </div>
                ...
                <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-6 hover:bg-zinc-800/30 hover:border-zinc-700 transition-all duration-300 hover:shadow-lg hover:shadow-black/20">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium text-white ${tripStatus.color}`}>
                            {tripStatus.label}
                          </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="flex items-center gap-3 p-3 bg-zinc-800/30 rounded-lg">
                      <MapPin className="text-lime-400 flex-shrink-0" size={18} />
                      <div>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-zinc-800/30 rounded-lg">
                      ...
                      <div>
                        ...
                        <p className="text-white font-medium">
                          {format(new Date(trip.startDate), "dd/MM/yyyy", { locale: ptBR })}
                        </p>
                        <p className="text-zinc-300 text-sm">
                          {format(new Date(trip.startDate), "EEEE", { locale: ptBR })}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 p-3 bg-zinc-800/30 rounded-lg">
                      ...
                      <div>
                        <p className="text-sm text-zinc-400">Término</p>
                        <p className="text-white font-medium">
                          {format(new Date(trip.endDate), "dd/MM/yyyy", { locale: ptBR })}
                        </p>
                        <p className="text-zinc-300 text-sm">
                          {format(new Date(trip.endDate), "EEEE", { locale: ptBR })}
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="text-center py-16">
          <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-8 max-w-md mx-auto">
            ...
            <p className="text-zinc-400 mb-6">
              Que tal planejar sua primeira aventura?
            </p>
            <div className="w-auto">
              <Button 
                colorScheme="primary"
